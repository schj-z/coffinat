...
;(function (root, factory) {
  ...
  const DAILY_LIMIT_MG = 400 // EFSA: habitual adult intake up to 400 mg/day is not a safety concern
  const SINGLE_DOSE_CAUTION_MG = 200 // EFSA: single doses up to 200 mg raise no concern

  /** Elimination rate constant (/h) from an elimination half-life in hours. */
  function keFromHalfLife(halfLifeH) {
    if (!(halfLifeH > 0)) return NaN
    return Math.LN2 / halfLifeH
  }

  /** Time of peak concentration (hours after intake). Undefined when ka === ke. */
  function tMaxHours(kaPerH, keH) {
   ...
   */
  function doseConcentrationMgL(doseMg, tauH, vdL, keH, kaPerH) {
  ...
  function volumeOfDistributionL(massKg) {
   ...
   */
  function concentrationAtMgL(doses, xMin, profile) {
  ...
  function concentrationSeriesMgL(doses, profile, xsMin) {
    const vdL = volumeOfDistributionL(profile.massKg)
    const keH = keFromHalfLife(profile.halfLifeH)
  ...
  const BEAN_POOL_MG_PER_G = { arabica: 13, robusta: 24, blend: 16 }
  ...
  const EXTRACTION_K_PER_MIN = 1.5 // ~95% of E_max reached by ~2 min
  const GROUNDS_RETENTION_ML_PER_G = 2 // water soaked up by spent grounds
   ...
   */
  function clockToWindowAbs(min, startMin) {
  ...
  return {
    ...
    keFromHalfLife,
    tMaxHours,
    doseConcentrationMgL,
    volumeOfDistributionL,
    concentrationAtMgL,
    concentrationSeriesMgL,
    BEAN_POOL_MG_PER_G,
    BREW_METHODS,
    EXTRACTION_K_PER_MIN,
    GROUNDS_RETENTION_ML_PER_G,
    brewCaffeine,
    WINDOW_START_MIN,
    WINDOW_MINUTES,
    parseClockToMinutes,
